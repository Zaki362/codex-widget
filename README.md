...
4. 执行本地安装脚本：
  ...
  - 移除 `/Applications/CodexQuota.app` 中可能存在的旧版本，避免 WidgetKit 加载旧扩展
  - 刷新 Launch Services 与 WidgetKit 注册，减少旧小组件卡住的概率
  - 启动菜单栏应用
...
一般情况下等待 30-60 秒即可看到更新。如果小组件看起来卡住，可以点击小组件触发一次手动刷新；如果仍然没有变化，可以移除小组件后重新添加。

## 更新日志

### v1.0.20

- 加固小组件点击刷新：整块小组件同时使用 `Link` 和 `widgetURL` 打开 `codexquota://refresh`
- 菜单栏 App 被重新打开时也会触发刷新，避免 URL 回调未送达时点击无效
- 安装脚本会清理废纸篓和构建目录中的旧注册，并重启 WidgetKit timeline 服务，修复桌面小组件卡在旧状态的问题

### v1.0.19

- 新增小组件点击手动刷新：点击桌面小组件会通过 `codexquota://refresh` 唤醒菜单栏应用并立即刷新本地快照
- 小组件刷新时间旁新增轻量刷新图标，提示可以点击触发刷新

### v1.0.18

- 修复额度窗口真实重置后，小组件可能继续保留旧百分比的问题
- 优化额度稳定保护逻辑：只保留疑似异常的单个额度项，不再回滚整份快照
- 当上一个 reset 时间已经过期，或新的 reset 已进入未来窗口时，接受最新日志中的额度数据
- 安装脚本会清理 `/Applications/CodexQuota.app` 中的旧副本，避免系统继续加载旧 Widget 扩展
...
可以先点击小组件触发一次手动刷新。一般等待 30-60 秒即可。如果仍然不变，可以移除桌面小组件后重新添加。
